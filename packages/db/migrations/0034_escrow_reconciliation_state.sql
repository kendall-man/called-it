-- Finalized account reconciliation is authoritative for transient market
-- state too. Freeze and unfreeze instructions do not have their own durable
-- projection rows, so mirror state and epoch only when this snapshot remains
-- the newest accepted reconciliation check.
...
begin
  -- Keep a rolling engine deploy compatible: old workers omit both fields.
