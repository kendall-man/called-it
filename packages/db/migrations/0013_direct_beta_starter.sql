-- The direct devnet beta has no per-group activation workflow. An explicit
-- disabled row remains a kill switch, but an absent row must not close a call.
-- A starter grant is once per user by its own unique grant row; legacy test
-- ledger and position history must not make a user ineligible.
...
begin
  ...
  v_has_history := exists (select 1 from wager_starter_grants where user_id = p_user_id);
