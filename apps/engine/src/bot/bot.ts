...
export function registerBotHandlers(bot: Bot, h: HandlerCtx): void {
  ...
  // A group command can be the user's first interaction. Detection ignores
  // commands, while wallet/stake tables reference the users row, so ensure
  // group and user context before any command handler runs.
