...
describe('replay logging privacy', () => {
  ...
  it('announces completion only after the source confirms success and unlocks failures', async () => {
    const runtime = createTelegramFlowRuntime();
    let emit: ((event: MatchEvent) => Promise<void>) | undefined;
    ...
    const supervisor = new IngestSupervisor(runtime.deps, {
      onReplayEvent: async () => undefined,
    } as unknown as typeof runtime.settler);
    const finished: number[] = [];
    const failed: number[] = [];
    supervisor.onReplayFinished = (_groupId, fixtureId) => { finished.push(fixtureId); };
    ...
    expect(finished).toEqual([FIXTURE_ID]);
