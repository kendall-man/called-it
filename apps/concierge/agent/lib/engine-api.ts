...
export function telegramIdentity(
  ...
  const current = trustedTelegramPrincipal(auth['current']);
  const initiator = trustedTelegramPrincipal(auth['initiator']);
  const principal = current ?? initiator;
  if (!principal) return null;
  const attributesValue = principal['attributes'];
...
function trustedTelegramPrincipal(value: unknown): Record<string, unknown> | null {
  return isRecord(value) && value['authenticator'] === 'telegram-webhook' ? value : null;
}
