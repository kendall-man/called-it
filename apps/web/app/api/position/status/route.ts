...
import { getEscrowPositionStatus } from '@/lib/position-server';
...
export async function POST(request: Request): Promise<NextResponse> {
  ...
  try {
    const result = await getEscrowPositionStatus(await requestBody(request), accessToken);
    return response(result.status, result.body);
  } catch {
    return response(503, { error: 'rpc_unavailable' });
  }
}
