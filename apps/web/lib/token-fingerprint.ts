...
export function tokenFingerprint(input: string): string {
  ...
  view.setUint32(totalLength - 8, Math.floor(bitLength / 0x1_0000_0000));
  ...
  for (let offset = 0; offset < totalLength; offset += 64) {
    ...
    for (let index = 0; index < 64; index += 1) {
      ...
      const temp1 = (h + sum1 + choose + (K[index] ?? 0) + (words[index] ?? 0)) >>> 0;
  ...
  return [h0, h1, h2, h3, h4, h5, h6, h7].map(hexWord).join('');
}
